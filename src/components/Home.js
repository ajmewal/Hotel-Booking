...
function Home() {
  ...
  const fetchPlaces = async (category = 'ALL') => {
    const data = await fetch('http://localhost:5000/api/places/places', {
      method: "POST",
  ...
  useEffect(() => {
    if (params.get('category')) {
      fetchPlaces(params.get('category'))
      console.log("h1")
    } else {
      fetchPlaces()
      console.log("h2")
    ...
    const handleScrollRight = () => {
      if (buttons) {
        ...
        buttons.scrollBy({ left: 200, behavior: 'smooth' });
      }
    };
  ...
  return (
    <div>
      ...
      <div className='flex mt-5 mb-2 items-center h-[80px]'>
        <div ref={scrollLeftRef} className='mx-7 border border-gray-500 rounded-full p-1 arrows left'>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
            <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" />
          </svg>
        </div>
        <div ref={buttonsRef} className='flex gap-[0.6rem] buttons overflow-x-scroll ' >

          {categoris.map((links, index) => {
            return (
              <Link key={index} to={`/?category=${links.to}`}><div className='h-fit relative'>
                <span className='w-full h-full left-0 top-0 absolute'></span>
                <img src={links.image} height="24px" width="24px" alt="" />
                <p>{links.text}</p>
              </div></Link>)
          })}
        ...
        <div ref={scrollRightRef} className='mx-7 border border-gray-500 rounded-full p-1 arrows right' >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
      ...
      <div className='flex mb-24 justify-center'>
        <div className='w-[90vw] grid grid-col-2 md:grid-cols-3 lg:grid-cols-4'>
          {Places.length > 0 && Places.map(place => {
            return (<Link key={place._id} to={'/places/' + place._id}><div className="max-w-xs border rounded-lg overflow-hidden shadow-lg m-4">
              <img
                src={place.photos[0]}
                alt="Wadi Rum, Jordan"
                className="w-full"
